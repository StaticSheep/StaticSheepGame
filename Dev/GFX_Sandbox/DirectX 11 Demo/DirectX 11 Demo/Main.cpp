...
int WINAPI WinMain(HINSTANCE hInstance,
                   ...
                   int nCmdShow)
{
    HWND hWnd = nullptr; // Unique Handle for window ID
   
  UNREFERENCED_PARAMETER( hPrevInstance); // silences unreference param warning
  UNREFERENCED_PARAMETER(lpCmdLine);      // silences unreference param warning

  if(FAILED(InitWindow(hInstance, nCmdShow, hWnd))) // Check for success
    return 0;
  ...
  MSG msg = {0}; // msg struct holds return of peekmessage

    while(WM_QUIT != msg.message)                    // While window hasn't been closed
    {
        if(PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) // Querry message queue
        {
    ...
    return msg.wParam;                                // Tell OS to close window instance
}
...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow, HWND &hWnd)
{
  ...
  wcex.lpszClassName = L"Graphics";               // Name of class
  if(!RegisterClassEx(&wcex))                     // Register class with OS
    return E_FAIL;                                // Error if failed

  // Create Window
  RECT rc = {0, 0, SCREEN_WIDTH, SCREEN_HEIGHT};                     // Defines rectangle dimensions for window
  ...
  hWnd = CreateWindow(L"Graphics",         // Name of window class
                      L"Hello World",      // Text on top bar
  ...
  if(!hWnd)                                // If app was not given unique handle: failed
    return E_FAIL;
