...
namespace dit {
    ...
    template <typename T>
    T& HashMap<T>::get(uint64 key, T& default_value)
    {
    ...
    template <typename T>
    void HashMap<T>::set(uint64 key, T&& value)
    {
        if (_hash.size() == 0)
            grow();
    ...
    template<typename T>
    size_t HashMap<T>::add_entry(uint64 key)
    {
    ...
    template<typename T>
    size_t HashMap<T>::find_or_fail(uint64 key) const
    {
        return find(key).data_i;
    }

    template<typename T>
    size_t HashMap<T>::find_or_make(uint64 key)
    {
        const FindResult fr = find(key);
    ...
    template <typename T>
    void HashMap<T>::find_and_erase(uint64 key)
    {
        const FindResult fr = find(key);
        if (fr.data_i != END_OF_LIST)
            erase(fr);
